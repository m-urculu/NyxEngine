...
void Engine::init() {
    Logger::init();
    LOG_INFO("=== Talos v0.2.0 (Phase 3A) ===");
...
Entity Engine::createMeshEntity(Mesh* mesh, Texture* texture,
    ...
    mat.texture = texture;
    mat.descriptorSet = m_descriptors.allocateMaterialSet(m_vulkanContext.getDevice(), *texture);
...
void Engine::loadGltfScene(const std::string& filepath) {
    ...
    for (size_t i = 0; i < meshDatas.size(); i++) {
        ...
        createMeshEntity(mesh, tex, {0.0f, 1.5f, -2.0f});
    }
}
